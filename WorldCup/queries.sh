...
echo "$($PSQL "select name from teams left join games on teams.team_id = games.winner_id where year=2014 and round='Eighth-Final' UNION ALL select name from teams left join games on teams.team_id = games.opponent_id where year=2014 and round='Eighth-Final' order by name")"
